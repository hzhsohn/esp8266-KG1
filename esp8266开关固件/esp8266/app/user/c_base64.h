...
static long int ICACHE_FLASH_ATTR base64_encode( char *src,long int src_len, char *dst)
{
    long int i = 0, j = 0;
    //  printf("%ld\n",src[2]);
...
static int ICACHE_FLASH_ATTR _base64Decode( unsigned char *input, unsigned int input_len, unsigned char *output, unsigned int *output_len )
{	
...
static int ICACHE_FLASH_ATTR base64Decode(unsigned char *in, unsigned int inLength, unsigned char **out)
{
	int len;
	...
	*out =(unsigned char*)os_malloc(len);
	(*out)[len-1]=0;
	if( *out ) {
